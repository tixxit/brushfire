...
object ModelType {
  ...
  /**
   * Returns a [[ModelType]] for a random forest generated by the Scalding
   * Trainer.
   *
   * @param extractor function to map a parameter map to a feature vector
   * @param voter     the voter to use when making predictions
   */
  def scaldingForest[K, V, T, P](
    extractor: Map[String, String] => Try[Map[K, V]],
    voter: Voter[T, Future[P]])(implicit
    jsonInj: JsonNodeInjection[Tree[K, V, T]],
    strInj: Injection[Tree[K, V, T], String],
    predInj: Injection[P, JsonNode]
  ): ModelType[JsonNode] = {
    val injections = new ForestModelInjections(extractor, voter)
    json[P, ForestModel[K, V, T, P], JsonNode](injections.jsonInjection, injections.scaldingCodec)(predInj(_))
  }

  def json[P, M <: Model[P], A](storeCodec0: Injection[M, A], codec0: Codec[M])(f: P => JsonNode): ModelType[A] =
    new ModelType[A] {
      case class JsonModel(model: M) extends Model[JsonNode] {
        def metadata: ModelMetadata = model.metadata
        def score(params: Map[String, String]): Future[JsonNode] = model.score(params).map(f)
      }

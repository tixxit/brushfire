...
class ForestModelTypeBuilder[K, V, T, P](
  ...
  def mapKeys[K1](implicit inj: Injection[K1, K]): ForestModelTypeBuilder[K1, V, T, P] =
    new ForestModelTypeBuilder(voter, extractor.mapKeys(inj))

  def mapValues[V1](implicit inj: Injection[V1, V], ord: Ordering[V]): ForestModelTypeBuilder[K, V1, T, P] =
    new ForestModelTypeBuilder(voter, extractor.mapValues(inj, ord))
  ...
  def dispatched[A, B, C, D](implicit
    ...
    new ForestModelTypeBuilder(voter, new DispatchedFeatureExtractor(extractor)(injA, injB, injC, injD, ord))
